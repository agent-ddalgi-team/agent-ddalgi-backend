...
| BE-08 | BE-06, BE-07 | 실제 배치 미리보기·검사·승인 스냅샷 출력·다운로드. 실패 재시도/같은 파일 재다운로드에 AI 재생성 없음. 실제 결과로 승인 검사 교체. | **PDF 완주 DONE (2026-09-27, 6.9절)** · **DOCX 승인·Export·다운로드 미완료**(overflow required·not_checked → 차단 유지, ㉞ 정책 변경 제안만 기록). LayoutCheck Job·미리보기·승인 ⑥ 실제 행·불변 artifact·Export·다운로드·공개 허가·재시작 복구 |
...
### 6.9 BE-08 배치 검사·승인 연결·출력·다운로드 (2026-09-27)
...
**6.9-1 API** (`/api/v1`, 멱등 규칙 BE-05 방식. 접근·세션·현재 승인/버전·공개 허가·artifact 검사가 캐시 응답보다 먼저)
